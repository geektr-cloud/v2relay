...
export const useTagStore = defineStore("tags", () => {
  ...
  const get = (id: string) => apiFetch<Tag>(`/tags/${encodeURIComponent(id)}`);

  const create = (input: { name: string; keywords?: string[] }) =>
    apiFetch<Tag>("/tags", { method: "POST", body: input });

  const update = (id: string, patch: Partial<{ name: string; keywords: string[] }>) =>
    apiFetch<Tag>(`/tags/${encodeURIComponent(id)}`, {
      method: "PATCH",
      body: patch,
    });

  const remove = (id: string) => apiFetch<void>(`/tags/${encodeURIComponent(id)}`, { method: "DELETE" });
